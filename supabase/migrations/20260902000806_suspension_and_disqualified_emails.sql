-- Both of these are bad news, and both are the kind of bad news people argue
-- with, so each says exactly what happened, when it ends, and what to do about
-- it. A refusal nobody can understand becomes a phone call to the store.
...
begin
  ...
  if n.template = 'confirmation' then
    ...
    select * into s from public.suspensions
     where trigger_reservation_id = r.id
     order by created_at desc limit 1;
    subj := 'You''re paused for 90 days — City Jeans drops';
    body := 'Hi ' || fname || E',\n\n'
         || 'Your last two reservations weren''t collected, so you can''t book '
         || 'or enter a drop for 90 days.' || E'\n\n'
         || case when s.until is not null then 'You can book again from '
              || to_char(s.until at time zone 'America/New_York', 'Mon DD, YYYY') || '.' || E'\n\n'
            else '' end
         || 'Reserved pairs sit behind the counter instead of on the floor, so '
         || 'a pair nobody picks up costs someone else the shot at it. If '
         || 'something came up both times, reply to this email and tell us — a '
         || 'manager can lift this.' || E'\n\n'
         || 'Nothing else changes: you can shop in store and online as normal.';
  else
    return null;
  end if;
