-- Reserved test domains (RFC 2606) must never reach a real mail provider:
-- every one bounces, and bounce rate is what SES suspends an account over.
-- Enforced in the claim step so it holds no matter what queues the row.
...
begin
  ...
  update public.notifications
     set status = 'skipped', locked_at = null,
         error = 'reserved test domain — never sent'
   where status = 'queued'
     and channel = 'email'
