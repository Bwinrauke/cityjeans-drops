-- A pair nobody collected is a pair that sat behind the counter all week
-- instead of selling. Marking that is not a job for a cashier on a Saturday,
-- so it happens on its own — one day after the window shuts, which covers the
-- customer who turns up the next morning.
create or replace function public.mark_missed_pickups(p_grace interval default interval '24 hours')
returns int
...
begin
  update public.reservations r
   ...
   where rel.id = r.release_id
     and r.status = 'confirmed'
     and r.archived_at is null
     and rel.pickup_ends_at is not null
     and rel.pickup_ends_at < now() - p_grace;
  get diagnostics v_n = row_count;
  return v_n;
end;
$$;

-- Two misses in a row, and they sit out ninety days. In a row is the point:
-- one pickup anywhere in between wipes the slate, so a single bad week does
...
begin
  ...
  loop
    -- the two most recent settled bookings for this person, newest first
    ...
    if v_last.status <> 'no_show' or v_prev.status is distinct from 'no_show' then
      continue;
    end if;

    -- already dealt with: either they are serving it, or a manager lifted it
    -- early and this same pair of misses must not put them straight back
    if exists (select 1 from public.suspensions s
                where s.trigger_reservation_id = v_last.id) then
      continue;
    end if;
    if public.active_suspension(p.email_norm, p.phone_norm) is not null then
      continue;
    end if;
...
-- One entry point for the nightly job.
create or replace function public.run_no_show_sweep()
returns jsonb
language plpgsql security definer set search_path = public, pg_temp
as $$
declare v_marked int; v_susp int;
begin
  v_marked := public.mark_missed_pickups();
  v_susp   := public.apply_no_show_suspensions();
  return jsonb_build_object('ok', true, 'marked_no_show', v_marked,
                            'suspended', v_susp, 'ran_at', now());
end;
$$;

revoke execute on function public.mark_missed_pickups(interval) from anon, authenticated;
revoke execute on function public.apply_no_show_suspensions(int) from anon, authenticated;
revoke execute on function public.run_no_show_sweep() from anon;
