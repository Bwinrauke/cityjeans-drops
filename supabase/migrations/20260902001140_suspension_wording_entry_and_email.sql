-- Same date fix on the raffle side and in the notice itself.
...
-- and the notice quotes the same first-working-day
create or replace function public.suspension_notice_body(p_first_name text, p_until timestamptz)
returns text
language sql stable set search_path = public, pg_temp
as $$
  select 'Hi ' || p_first_name || E',\n\n'
      || 'Your last two reservations weren''t collected, so you can''t book '
      || 'or enter a drop for 90 days.' || E'\n\n'
      || 'You can book again from '
      || to_char(public.eligible_again(p_until), 'Mon FMDD, YYYY') || '.' || E'\n\n'
